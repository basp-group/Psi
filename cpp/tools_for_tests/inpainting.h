#ifndef PSI_TOOLS_FOR_TESTS_INPAINTING_H
#define PSI_TOOLS_FOR_TESTS_INPAINTING_H
...
template <class T>
Vector<T> target(psi::LinearTransform<Vector<T>> const &sampling, psi::Image<T> const &image) {
  return sampling * Vector<T>::Map(image.data(), image.size());
}

template <class T>
typename real_type<T>::type
sigma(psi::LinearTransform<Vector<T>> const &sampling, psi::Image<T> const &image) {
  auto const snr = 30.0;
...
Vector<T> dirty(psi::LinearTransform<Vector<T>> const &sampling, psi::Image<T> const &image,
                RANDOM &mersenne) {
...
template <class T>
typename real_type<T>::type
epsilon(psi::LinearTransform<Vector<T>> const &sampling, psi::Image<T> const &image) {
  auto const y0 = target(sampling, image);
