...
int main(int argc, char const **argv) {
  ...
  psi::t_uint const block_number = 1;
  psi::t_uint const imsizey = image.rows();
  psi::t_uint const imsizex = image.cols();
  ...
  PSI_HIGH_LOG("Starting primal dual");
  ...
  PSI_HIGH_LOG("PSI-primal-dual converged in {} iterations", diagnostic.niters);
  if(output != "none")
