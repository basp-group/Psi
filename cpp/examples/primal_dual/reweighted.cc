...
int main(int argc, char const **argv) {
  ...
  PSI_MEDIUM_LOG("Computing primaal-dual parameters");
  ...
  auto sigma2 = 1e0 / nu2;
    ...
    PSI_HIGH_LOG("Creating primal-dual Functor");
  auto pd = psi::algorithm::PrimalDual<Scalar>(y)
                         .itermax(500)
                         .tau(tau)
                         .kappa(kappa)
                         .sigma1(sigma1)
  ...
  auto reweighted
      = psi::algorithm::reweighted(pd).itermax(5).min_delta(min_delta).is_converged(
          psi::RelativeVariation<Scalar>(1e-3));

  PSI_MEDIUM_LOG("Starting primal dual");
  // Here, we default to (Φ^Ty/ν, ΦΦ^Ty/ν - y)
  auto diagnostic = reweighted();
  PSI_MEDIUM_LOG("primal-dual returned {}", diagnostic.good);

  PSI_MEDIUM_LOG("PSI-primal-dual converged in {} iterations", diagnostic.niters);
  if(output != "none")
