...
int main(int argc, char const **argv) {
	...
	for(int f=0; f<frequencies; f++){
		target[f][0] = Phitemp[f][0] *t_Vector::Map(image.data(), image.size());
		...
		for(psi::t_int i = 0; i < target[f][0].size(); i++)
	...
	if(output != "none") {
		for(int f=0; f<frequencies; f++){
			t_Vector const dirty = Phi[f][0]->adjoint() * target[f][0];
	...
	auto const wavelet = psi::wavelets::factory("DB4", 4, Decomp.my_lower_root_wavelet());

	std::vector<psi::LinearTransform<psi::Vector<psi::t_complex>>> Psi(frequencies,psi::linear_transform_identity<Scalar>());
	for(int f=0; f<Decomp.my_number_of_frequencies(); f++){
		Psi[f] = psi::linear_transform<psi::t_complex>(wavelet, image.rows(), image.cols());
	}
	...
	if(Decomp.my_number_of_root_wavelets()>0){
		Psi_Root = psi::linear_transform<psi::t_complex>(wavelet, image.rows(), image.cols());
	}
	...
	PSI_HIGH_LOG("Creating wideband primal-dual");
	auto wideband_ppd
	...
	PSI_HIGH_LOG("Starting re-weighted wideband primal dual from psi library");
	auto diagnostic = wideband_ppd();

	PSI_MEDIUM_LOG("Wideband primal-dual returned {}", diagnostic.good);

	PSI_MEDIUM_LOG("Wideband primal-dual converged in {} iterations", diagnostic.niters);

	if(output != "none"){
		for(int f=0; f<frequencies; f++){
			Matrix main_image = Matrix::Map(diagnostic.x.col(f).data(), image.rows(), image.cols());
