...
TEST_CASE("Primal Dual, testing norm(output - target()) < l2ball_epsilon()", "[primaldual][integration]") {
  ...
  auto primaldual = algorithm::PrimalDual<Scalar>(target)
                         .Phi(mId)
                         .Psi(mId)
                         .itermax(5000)
                         .kappa(0.1)
...
template <class T>
struct is_primal_dual_ref : public std::is_same<psi::algorithm::PrimalDual<double> &, T> {};
TEST_CASE("Check type returned on setting variables") {
  ...
  CHECK(is_primal_dual_ref<decltype(pd.l1_proximal_weights(Vector<psi::t_real>::Zero(0)))>::value);
