...
TEST_CASE("Check SARA implementation mechanically", "[wavelet]") {
  ...
  typedef std::tuple<std::string, psi::t_uint> t_i;
...
TEST_CASE("Check SARA implementation mechanically with total wavelet size", "[wavelet]") {
  using namespace psi::wavelets;
  using namespace psi;

  typedef std::tuple<std::string, psi::t_uint> t_i;
  SARA const sara({t_i{std::string{"DB3"}, 1u}, t_i{std::string{"DB1"}, 2u},
                  t_i{std::string{"DB1"}, 3u}}, 3);
  SECTION("Construction and vector functionality") {
    CHECK(sara.size() == 3);
    CHECK(sara[0].levels() == 1);
    CHECK(sara[1].levels() == 2);
...
TEST_CASE("Linear-transform wrapper with total wavelet size", "[wavelet]") {
  using namespace psi::wavelets;
  using namespace psi;
  SARA const sara({std::make_tuple(std::string{"DB3"}, 1u), std::make_tuple(std::string{"DB1"}, 2u),
                  std::make_tuple(std::string{"DB1"}, 3u)}, 3);
