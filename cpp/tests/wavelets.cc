...
typedef psi::Array<psi::t_uint> t_iVector;
...
t_iVector random_ivector(psi::t_int size, psi::t_int min, psi::t_int max) {
  extern std::unique_ptr<std::mt19937_64> mersenne;
...
template <class T0>
void check_round_trip(Eigen::ArrayBase<T0> const &input_, psi::t_uint db,
                      psi::t_uint nlevels = 1) {
  auto const input = input_.eval();
  auto const &dbwave = psi::wavelets::daubechies_data(db);
  auto const transform = psi::wavelets::direct_transform(input, nlevels, dbwave);
  auto const actual = psi::wavelets::indirect_transform(transform, nlevels, dbwave);
  CAPTURE(actual);
  CAPTURE(input);
  CAPTURE(transform);
  CHECK(input.isApprox(actual, 1e-14));
  CHECK(not transform.isApprox(psi::wavelets::direct_transform(input, nlevels - 1, dbwave), 1e-4));
}

TEST_CASE("Wavelet transform innards with integer data", "[wavelet]") {
  using namespace psi::wavelets;
