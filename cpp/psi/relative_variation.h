#ifndef PSI_RELATIVE_VARIATION_H
#define PSI_RELATIVE_VARIATION_H
...
public:
  ...
  typedef typename Eigen::Array<Scalar, Eigen::Dynamic, Eigen::Dynamic> t_Array;
  //! Maximum variation from one step to the next
  RelativeVariation(Real tolerance = 1e-12, std::string const &name = "")
      : tolerance_(tolerance), previous_(t_Array(0,0)), name_(name){}; // previous_(typename Array<Scalar>::Index(0))
  //! Copy constructor
...
bool RelativeVariation<SCALAR>::operator()(Eigen::ArrayBase<T> const &input) {
  ...
  previous_ = input;
  PSI_LOW_LOG("    - {} relative variation: {} <? {}", name(), std::sqrt(norm), tolerance());
...
template <class SCALAR> bool ScalarRelativeVariation<SCALAR>::operator()(Scalar const &current) {
  ...
  PSI_LOW_LOG("    - {} relative variation: {} < {} * {} or {} < {} is {}", name(), diff,
