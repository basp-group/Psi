#ifndef PSI_CREDIBLE_REGION_H
#define PSI_CREDIBLE_REGION_H
...
t_real compute_energy_upper_bound(
    ...
    const std::function<t_real(typename T::PlainObject)> &objective_function) {
  if(alpha <= 0)
    PSI_THROW("α must positive.");
  if(alpha >= 1)
    PSI_THROW("α must less than 1.");
  ...
  PSI_MEDIUM_LOG("Confidence interval: %{}", 100 * alpha);
  PSI_MEDIUM_LOG("γ = {}, g(x_s) = {}", gamma, energy);
  return gamma;
}
...
find_credible_interval(const Eigen::MatrixBase<T> &solution, const t_uint &rows, const t_uint &cols,
  ...
  if(solution.size() != cols * rows)
    PSI_THROW("Solution is wrong size for credible interval.");
  if((std::get<2>(region) > rows) or (std::get<3>(region) > cols))
    PSI_THROW("Region is out of bounds.");
  if(energy_upperbound <= 0)
    PSI_THROW("Energy upper bound is not positive when calculating credible interval.");
  ...
  *varried_solution = Matrix<typename T::Scalar>::Map(varried_solution->data(), rows, cols);
  ...
  const t_real bound_lower
      = psi::bisection_method(energy_upperbound, bound_estimater, -b, 0., 1e-3);
  const t_real bound_upper
      = psi::bisection_method(energy_upperbound, bound_estimater, 0., b, 1e-3);
...
credible_interval_grid(const Eigen::MatrixBase<T> &solution, const t_uint &rows, const t_uint &cols,
  ...
  if((std::get<0>(grid_pixel_size) > rows) or (std::get<1>(grid_pixel_size) > cols))
    PSI_THROW("Grid pixel size too big.");
  typedef typename T::PlainObject Derived;
  ...
  PSI_LOW_LOG("Starting calculation of credible interval: {} x {} grid.", grid_rows, grid_cols);
  for(t_uint i = 0; i < grid_rows; i++) {
    for(t_uint j = 0; j < grid_cols; j++) {
      ...
      if(static_cast<t_int>(rows - start_row - drow) < 0)
        PSI_THROW("Interval grid calculation going out of bounds.");
      ...
      const t_uint delta_col
          = ((dcol > (cols - start_col - dcol)) and ((cols - start_col - dcol) > 0)) ?
                cols - start_col - dcol :
                dcol;
      PSI_LOW_LOG("Grid pixel ({}, {}): [{}, {}) x [{}, {})", i, j, start_row,
      ...
      PSI_LOW_LOG("η- = {}, mean = {}, η+ = {}", std::get<0>(bounds), std::get<1>(bounds),
                   std::get<2>(bounds));
