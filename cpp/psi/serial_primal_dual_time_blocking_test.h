...
void SerialPrimalDualTimeBlocking<SCALAR>::iteration_step(t_Vector &out, std::vector<t_Vector> &residual, t_Vector &u,
	...
	// garbage/null/zero.
	x_hat = Phi()[0]->FFT(image_bar); // issue size x_hat here !

    // Extract the portion from x_hat to be sent to the workers
    // std:vector<Eigen::SparseMatrix<t_complex>> x_hat_broadcast(decomp().my_frequencies()[0].number_of_time_blocks);
    // for(int i=0;i<decomp().my_frequencies()[0].number_of_time_blocks;i++)
    // {
    //     Eigen::SparseMatrix<t_complex> x_hat_i(Phi()[0]->imsizey(), Phi()[0]->imsizex();
	//     x_hat_i.reserve(Vector<t_int>::Constant(Phi()[0]->imsizey(), Phi()[0]->Ju() * Phi()[0]->Jv());
    //     for (int k=0; k<(Phi()[0]->G).outerSize(); ++k) // beware storage order between x_hat and G[i] (for the sparse matrices...): G stored in Row major!
    //     {
    //         for (Eigen::SparseMatrix<t_complex>::InnerIterator it((Phi()[i]->G()),k); it; ++it)
    //         {
    //             x_hat_i.coeffRef(it.index(), it.col()) = x_hat(it.index(), it.col()); // to be verified, check compatibility with the format of G (RowMajor order)
    //         }
    //     }
    //     x_hat_broadcast[i] = x_hat_i;
    // }

    std::vector<Eigen::SparseMatrix<t_complex>> x_hat_broadcast(decomp().my_frequencies()[0].number_of_time_blocks);
    for(int i=0;i<decomp().my_frequencies()[0].number_of_time_blocks;i++)
    {
        Vector<t_int> indices = Phi()[i]->get_fourier_indices(); // to be transmitted to the master node 
        Eigen::SparseMatrix<t_complex> x_hat_i(Phi()[i]->oversample_factor()*Phi()[i]->imsizey() * Phi()[i]->oversample_factor()*Phi()[i]->imsizex(), 1);
	    x_hat_i.reserve(indices.rows());
        for (int k=0; k<indices.rows(); k++)
        {
            x_hat_i.insert(indices(k),0) = x_hat(indices(k),0);
        }
		x_hat_i.makeCompressed();
        x_hat_broadcast[i] = x_hat_i; // to be transmitted back to the worker i
    }
	...
	for(int i=0;i<decomp().my_frequencies()[0].number_of_time_blocks;i++){
		...
		t_Vector v_prox;
		if(preconditioning()){
			temp = v[i] + (Phi()[i]->G_function(x_hat_broadcast[i])).eval().cwiseProduct(Ui()[i]);
			...
			temp = v[i] + (Phi()[i]->G_function(x_hat_broadcast[i])).eval();
