...
soft_threshhold(Eigen::DenseBase<T0> const &input, Eigen::DenseBase<T1> const &threshhold) {
  if(input.size() != threshhold.size())
    PSI_THROW("Threshhold and input should have the same size");
...
//! Computes weighted L21 norm (L2 along the rows)
template <class T0, class T1>
typename real_type<typename T0::Scalar>::type
l21_norm(Eigen::MatrixBase<T0> const &input, Eigen::MatrixBase<T1> const &weights) {
  if(weights.size() == 1)
    return input.rowwise().norm().sum() * std::abs(weights(0));
  return (input.rowwise().norm()).cwiseProduct(weights).sum();
}
//! Computes L21 norm (L2 along the rows)
template <class T0>
typename real_type<typename T0::Scalar>::type 
l21_norm(Eigen::MatrixBase<T0> const &input) {
  return (input.rowwise().norm()).sum();
}

//! Computes nuclear norm of a matrix
template <class Derived>
typename real_type<typename Derived::Scalar>::type nuclear_norm(Eigen::MatrixBase<Derived> const &input) {
  return (input.bdcSvd().singularValues()).cwiseAbs().sum();
}
//! Computes weighted nuclear norm of a matrix
template <class T0, class T1>
typename real_type<typename T0::Scalar>::type nuclear_norm(Eigen::MatrixBase<T0> const &input, Eigen::MatrixBase<T1> const &weights) {
  if(weights.cols() != 1)
    PSI_THROW("Weights should be given as a column vector");
  if(weights.rows() != input.cols())
    PSI_THROW("Number of weighting coefficients and of column of the input should be the same");
  typename Eigen::BDCSVD<typename T0::PlainObject> svd(input, Eigen::ComputeThinU | Eigen::ComputeThinV);
  return (svd.singularValues()).cwiseAbs().cwiseProduct(weights).sum();
}
// //! Computes nuclear norm when the singular values are given as an input
// template <class T>
// typename real_type<typename T::Scalar>::type
// nuclear_norm(Eigen::MatrixBase<T> const &singular_values) {
//   return singular_values.cwiseAbs().sum();
// }
//! Computes weighted nuclear norm when the singular values are given as input
template <class T>
typename real_type<typename T::Scalar>::type
nuclear_norm(Eigen::MatrixBase<T> const &singular_values, Eigen::MatrixBase<T> const &weights) {
  return singular_values.cwiseAbs().cwiseProduct(weights).sum();
}
