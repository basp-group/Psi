#ifndef PSI_L1_PROXIMAL_H
#define PSI_L1_PROXIMAL_H
...
public:
  ...
  PSI_MACRO(Psi, LinearTransform<Vector<Scalar>>);
...
#undef PSI_MACRO
  ...
  L1TightFrame<Scalar> &weights(Real const &value) {
    if(value <= 0e0)
      PSI_THROW("Weight cannot be negative or null");
    weights_ = Vector<Real>::Ones(1) * value;
    return *this;
  }

  //! Set Ψ and Ψ^† using arguments that psi::linear_transform understands
  template <class... ARGS>
...
operator()(Eigen::MatrixBase<T0> &out, Real gamma, Eigen::MatrixBase<T1> const &x) const {
  ...
  PSI_LOW_LOG("Prox L1: objective = {}", objective(x, out, gamma));
}
...
L1TightFrame<SCALAR>::objective(Eigen::MatrixBase<T0> const &x, Eigen::MatrixBase<T1> const &z,
                                Real const &gamma) const {
  return 0.5 * (x - z).squaredNorm() + gamma * psi::l1_norm(Psi().adjoint() * z, weights());
}
...
public:
  ...
  PSI_MACRO(real_constraint, bool);
  //! Whether to do fista mixing or not
  PSI_MACRO(fista_mixing, bool);
#undef PSI_MACRO
...
operator()(Eigen::MatrixBase<T0> &out, Real gamma, Vector<Scalar> const &x, MIXING mixing) const {
  ...
  PSI_MEDIUM_LOG("  Starting Proximal L1 operator:");
  t_uint niters = 0;
  out = x;

  Breaker breaker(objective(out, x, gamma), tolerance(), false); // not fista_mixing());
  PSI_LOW_LOG("    - iter {}, prox_fval = {}", niters, breaker.current());
  ...
  for(++niters; niters < itermax() or itermax() == 0; ++niters) {
    ...
    PSI_LOW_LOG("    - iter {}, prox_fval = {}, rel_fval = {}", niters, breaker.current(),
  ...
  if(breaker.two_cycle())
    PSI_WARN("Two-cycle detected when computing L1");

  if(breaker.converged()) {
    PSI_LOW_LOG("  Proximal L1 operator converged at {} in {} iterations", breaker.current(),
                 niters);
  } else
    PSI_ERROR("  Proximal L1 operator did not converge after {} iterations", niters);
