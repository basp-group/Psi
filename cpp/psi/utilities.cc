...
Image<> read_tiff(std::string const &filename) {
  PSI_MEDIUM_LOG("Reading image file {} ", filename);
  TIFF *tif = TIFFOpen(filename.c_str(), "r");
  if(not tif)
    PSI_THROW("Could not open file ") << filename;
  ...
  PSI_LOW_LOG("- image size {}, {} ", width, height);
  Image<> result = Image<>::Zero(height, width);

  uint32 *raster = (uint32 *)_TIFFmalloc(width * height * sizeof(uint32));
  if(not raster)
    PSI_THROW("Could not allocate memory to read file ") << filename;
  if(not TIFFReadRGBAImage(tif, width, height, raster, 0))
    PSI_THROW("Could not read file ") << filename;
...
void write_tiff(Image<> const &image, std::string const &filename) {
  PSI_MEDIUM_LOG("Writing image file {} ", filename);
  PSI_LOW_LOG("- image size {}, {} ", image.rows(), image.cols());
