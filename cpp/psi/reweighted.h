#ifndef PSI_REWEIGHTED_H
#define PSI_REWEIGHTED_H
...
reweighted(ALGORITHM const &algo, typename Reweighted<ALGORITHM>::t_Reweightee const &reweightee);
...
public:
  ...
  Reweighted(Algorithm const &algo, t_Reweightee const &reweightee)
      : algo_(algo), reweightee_(reweightee), itermax_(std::numeric_limits<t_uint>::max()),
...
operator()(ReweightedResult const &warm) const {
  PSI_HIGH_LOG("Starting reweighted scheme");
  ...
  for(result.niters = 0; result.niters < itermax(); ++result.niters) {
    PSI_LOW_LOG("Reweighting iteration {}/{} ", result.niters, itermax());
    PSI_LOW_LOG("  - delta: {}", delta);
    result.weights = delta / (delta + reweightee(result.algo.x).array().abs());
    result.algo = algo(result.algo);
    if(is_converged(result.algo.x)) {
      PSI_MEDIUM_LOG("Reweighting scheme did converge in {} iterations", result.niters);
      result.good = true;
      break;
    }
  ...
  if(not is_converged())
    ...
    PSI_ERROR("Reweighting scheme did *not* converge in {} iterations", itermax());
  return result;
}
...
template <class SCALAR> class PrimalDualDebug;
template <class SCALAR>
Reweighted<PositiveQuadrant<PrimalDualDebug<SCALAR>>>
reweighted(PrimalDualDebug<SCALAR> const &algo) {
...
template <class SCALAR> class PrimalDualBlocking;
template <class SCALAR>
Reweighted<PositiveQuadrant<PrimalDualBlocking<SCALAR>>>
reweighted(PrimalDualBlocking<SCALAR> const &algo) {
